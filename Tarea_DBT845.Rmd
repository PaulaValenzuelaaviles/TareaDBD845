...
```{r}
ct_his_elf <- CT_gene_e %>% filter(Gene=="elf") %>% ggplot(aes(x=ct))+
  geom_histogram(color="darkblue", fill="lightblue", bins=10)
ct_his_elf


ct_elf <- ggplot(CT_gene_e, aes(Tissue, ct))+
  geom_boxplot(aes(fill=Treatment))

ct_elf

ct_elf <- ggplot(CT_gene_e, aes(Program, ct))+
  geom_boxplot(aes(fill=Treatment))+
  facet_wrap(~ Tissue)

ct_elf

ct_elf <- ggplot(CT_gene_e, aes(fill=Time, x=Tissue, y=ct))+
...
chart.Correlation(Cor_ct_gene_HK_e[, 6:18], histogram=FALSE, pch=19, method = c("spearman"))
